...
def postprocess(
    ...
    **kwargs
    ...
    nc = 80 if kwargs.get('nc') is None else kwargs.get('nc')
...
def detect(image: np.ndarray, model: Model, nc: int):
    """
...
def run(det_compiled_model, image):
    detections = detect(image, det_compiled_model, len(label_map))[0]
    # print(f"{detections = }")
...
def main(label_map):
    # label_map = {0: 'bird', 1: 'person', 2: 'head', 3: 'plane'}
    ...
    while True:
        ...
        run(det_compiled_model, image)

        fps += 1/(time.perf_counter() - start_time)
        print(f"Avg. FPS = {fps/iter_count}")
    # img.show()


if __name__ == "__main__":
    det_model = YOLO('./models/pre-trained-models/yolov8n.pt')
    label_map = det_model.names

    
    IMAGE_PATH = "./images/samples/bus.jpg"
